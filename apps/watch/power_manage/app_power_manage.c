...
int app_sleep_check_timer=0;
static void app_sleep_check()
{
    int value;
    struct lp_target *p;
    list_for_each_lp_target(p){
    ...
    if(value){
        ++app_is_sleep;
        putchar('ooooooooooooooooooo');
        if(app_is_sleep > LV_LOW_POWER_ENTER_TIME){
            putchar('PPPPPPPPPPPPPPPPPP');
            power_event_value_to_user(POWER_EVENT_POWER_LOW,0x55);
            usr_timer_del(app_sleep_check_timer);
        }
    }   
    else{
        putchar('@@@@@@@@@@@@@@@@@@');
...
void auto_sleep_init()
{
#if LV_LOW_POWER_EN
    if (app_sleep_check_timer == 0) {
        app_sleep_check_timer = usr_timer_add(NULL, app_sleep_check, 1000, 0);//检查是否进入低功耗
    }
#endif
}
#endif
...
void power_event_value_to_user(u8 event,u8 value)
{
    struct sys_event e;
    e.type = SYS_DEVICE_EVENT;
    e.arg  = (void *)DEVICE_EVENT_FROM_POWER;
...
#endif
#if LV_LOW_POWER_EN
    r_printf("event:%d value:0x%x\n",dev->event,dev->value);
    if((dev->event == POWER_EVENT_POWER_LOW)&&(dev->value == 0x55))
    {
        uint8_t lowpower_entry_info = 0x55;
        syscfg_write(LOWPOWER_ENTRY_INFO, &lowpower_entry_info, sizeof(lowpower_entry_info));
    }
#endif
#if (LV_LOW_POWER_EN && !TCFG_SYS_LVD_EN)
    soft_poweroff_mode(1);  ///强制关机
    sys_enter_soft_poweroff(NULL);
#endif
