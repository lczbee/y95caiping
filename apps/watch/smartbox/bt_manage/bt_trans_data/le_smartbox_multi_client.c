...
#include "../lv_charging_case/lv_frame/custom/system/device_sleep_control.h"
...
#define SB_SCAN_TIME_CYCLE  5000    //  修复概率出现一直搜索功耗高BUG
...
extern u8 sbox_btmulti_client_init_flag;

// static int user_ble_scan_out_timer =0;
void user_check_into_scan(void)
{
    if(!sbox_btmulti_client_init_flag) {
        y_printf("sbox_btmulti_client_init has not been inited yet, return, func: %s, line: %d", __func__, __LINE__);
        return;
    }

#if SB_SCAN_TIME_CYCLE
   ...
   if(box_info.box_clid_status != local_cld_state){
        ...
        if(local_cld_state == 1){
            screen_on_source = SCREEN_ON_SOURCE_CLID;
#if LV_DEMO_POP_UP_EN
            screen_pop_up_callback_enter();
#endif /* LV_DEMO_POP_UP_EN */
            lv_exit_sleep();
        }else if(local_cld_state== 2){
#if LV_DEMO_POP_UP_EN
            pop_up_set_box_clid_last_status(local_cld_state);
#endif
            lv_enter_sleep();
        }
   }
...
#define ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE 0x0085 //耳机的notify handle

static void open_handle_ccc(void)
{
    g_printf("%s[line:%d]\n",__func__,__LINE__);
    u16 tmp_16 = 0x01;//fixed
    client_operation_send(ATT_CHARACTERISTIC_ae02_01_CLIENT_CONFIGURATION_HANDLE, &tmp_16, 2, ATT_OP_WRITE);
...
#define TEST_BOX_BLE_NAME_LEN	0xc
            ...
            if (check_device_is_match(CLI_CREAT_BY_NAME, adv_data_pt, lenght-1)) {
                find_remoter = 1;
                memcpy(custom_remote_addr, adv_address, 6);
...
#if SB_SCAN_TIME_CYCLE
    void ble_scan_sb_check_init();
    ble_scan_sb_check_init();
#endif
...
static u8 get_ble_work_test(){
    return ble_work_state[0];
}

static void ble_scan_check(void * p) {
    extern u8 get_ble_work_test();
    r_printf("ble state : %d", get_ble_work_test());

    static int cnt = 0;

    if(get_ble_work_test() == 64){
        cnt += SB_SCAN_TIME_CYCLE;
        r_printf(">>>>CNT %d", cnt);
    } else {
        cnt = 0;
    }

    if(cnt >= BLE_POWERON_PAGE_TIME){
        extern void ble_client_module_enable(u8 en);
        ble_client_module_enable(0);
        cnt = 0;
    }
...
void ble_scan_sb_check_init(){
    static int id = 0;
    if(!id){
        id = sys_timer_add(0, ble_scan_check, SB_SCAN_TIME_CYCLE);
    }
}
