...
#if     1//彩屏仓壁纸
            if(!strncmp("ANI",rename,3) || !strncmp("VIE",rename,3)) {
                char *selectwallper = getselectwallper();
                if(selectwallper) {
                    void * tmp = fopen(selectwallper+2, "r");
                    if(tmp){
                        r_printf("%s 存在，先删除再更新", selectwallper+2);
                        fclose(tmp);
                        printf("del is ok: %d",fdelete_by_name(selectwallper+2));
                    }
                }

                char root_rename[64] = "d:storage/virfat_flash/C/";
                strcat(root_rename, rename);
                setselectwallper(root_rename);
            }

            char checksamename[64] = "storage/virfat_flash/C/";
            strcat(checksamename, rename);
            void * tmp = fopen(checksamename, "r");
            if(tmp){
                r_printf("%s 存在，先删除再更新", checksamename);
                fclose(tmp);
            }
#endif
    ...
    if (ftp_d)	{
        if (status == JL_PRO_STATUS_SUCCESS)	{
            ...
            if (file_rename((const char *)data) == 0) {
                ...
                if (!dev_manager_online_check(ftp_d->dev, 1)) {
                    // 检测到是音频文件才标记设备有效
                    u8 ext[4] = {0};
                    extern int unicode_check(char *path);
                    extern int f_PickOutName(char **pPath, char *sfn, char *lfn, int max_len, int *is_dir);
                    extern bool ftype_compare(const char *types, const char *ext);
                    if (unicode_check(data)) {
                        printf("file name is unicode \n");
                        char *p = data;
                        int is_dir = 0;
                        char *tmp_ptr = zalloc(16 + (26 * 10));
                        ASSERT(tmp_ptr);
                        char *upcase_name = tmp_ptr;
                        char *lfn = tmp_ptr + 16;
                        f_PickOutName(&p, upcase_name, lfn, 26 * 10, &is_dir);
                        memcpy(ext, upcase_name + 8, 3);
                        free(tmp_ptr);
                    } else {
                        printf("file name is ASCII \n");
                    ...
                    printf("ext:%s \n", ext);
                    if (ftype_compare(DEC_EXT_NAME, ext)) {
                        printf("is music file \n");
                        dev_manager_set_valid(ftp_d->dev, 1);
                    }
                }
