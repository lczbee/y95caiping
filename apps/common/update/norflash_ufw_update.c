...
extern const int support_norflash_update_loader_only;
...
static u16 norflash_ufw_update_f_open(void)
{
    ...
    if (support_norflash_update_loader_only) {
        // 需要指定对应分配的区域
        /* dev_ptr = dev_open("ota_bin", NULL); */
    } else {
...
void norflash_update_loader_only_init(u32 norflash_addr)
{
    if (0 == support_norflash_update_loader_only) {
        return;
    }

    user_remote_base_addr = norflash_addr;
    // 填充需要在loader中获取的数据的物理起始地址和长度
    /* nor_update_info.src_file_addr = 0; */
    /* nor_update_info.src_file_len = 0; */
    ...
    exflash_move_loader_task_init(&nofrflash_ufw_update_info);
}
