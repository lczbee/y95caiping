...
local KEY_MSG_EVENT = cfg:enumMap("按键消息类型", KEY_MSG_EVENT_TABLE_CN);
...
function key_short_click_msg_cfg_add(prefix, cnt, key_en_item, key_num_item)
    ...
    key_short_click_msg_cfg:setTextOut(
        function (ty)
            if (ty == 3) then
                return "#define TCFG_KEY" .. cnt  .. "_KEY_SHORT_CLICK_MSG" .. TAB_TABLE[2] .. KEY_MSG_EVENT_TABLE_EN[key_short_click_msg_cfg.val] .. NLINE_TABLE[1]
            end
        end
    )
