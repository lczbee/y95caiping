...
extern int lcd_draw_area_for_imb(u8 index, u8 *lcd_buf, int left, int top, int width, int height, int wait); // 给imb专用
...
static void jpeg_api_task(void *arg)
{
    ...
    for (h = 0; h < info.height; h += pixel_lines) {
        ...
        lcd_draw_area_for_imb(0, out.outbuf, 0, h, info.width, pixel_lines, 0);
    }
