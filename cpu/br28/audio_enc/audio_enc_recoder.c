...
#define LADC_MIC_CH_NUM         2	//双mic通话
#else /*TCFG_AUDIO_DUAL_MIC_ENABLE == 0*/
#define LADC_MIC_CH_NUM         1	//单mic通话
...
#else /* #if RECORDER_AEC_EN */

#if (LADC_MIC_CH_NUM == 2)
        if (rec_hdl->nchl == 2) { // 双声道输出
            pcm2file_enc_write_pcm(rec_hdl->file, data, len * 2);
        } else { // 单声道输出
            pcm_dual_to_single(data, data, len * 2);
            pcm2file_enc_write_pcm(rec_hdl->file, data, len * 1);
        }
...
int recorder_encode_start(struct record_file_fmt *f)
{
    ...
    } else if (fmt.source == ENCODE_SOURCE_MIC) {
#if (LADC_MIC_CH_NUM == 2)
        size += REC_ALIN((sizeof(s16) * ADC_BUFS_SIZE), 4);
#endif /* #if (LADC_MIC_CH_NUM == 2)*/
    }
...
#if RECORDER_AEC_EN
    if ((fmt.channel != 1) || (fmt.sample_rate > 16000)) {
        printf("aec only support 8k,16k,1ch \n");
        free(rec);
        recorder_encode_clock_remove(fmt.coding_type);
        return -1;
    }
    /* int err = audio_aec_init(fmt.sample_rate); */
#if (LADC_MIC_CH_NUM == 2)
    int err = audio_aec_open(fmt.sample_rate, ANS_EN | ENC_EN | AGC_EN, adc_aec_output_cb);
#else
    int err = audio_aec_open(fmt.sample_rate, ANS_EN | AGC_EN, adc_aec_output_cb);
#endif
    if (err) {
