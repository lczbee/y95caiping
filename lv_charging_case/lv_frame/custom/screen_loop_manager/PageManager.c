...
static void PageMoveDistance(enum PageDir Dir, lv_coord_t Distance)
{
    ...
    if(PageManager.PublicDown != NULL) {
        if(PageManager.HomePage == PageManager.PublicDown && lv_obj_get_y(PageManager.PublicDown->obj) > 0) {
            if(PageManager.PublicDown->Up.Page->obj) {
                lv_obj_clear_flag(PageManager.PublicDown->Up.Page->obj, LV_OBJ_FLAG_HIDDEN);
            }   
        }
    }
...
static void AnimReady_cb(lv_anim_t * Anim)
{
    ...
    if(PM_Custom_Cb.status_bar_bottom_update) {
        PM_Custom_Cb.status_bar_bottom_update(PageManager.HomePage->id); // 动画结束回调，根据id去更新底部进度条
    }

    if(PageManager.PublicDown != NULL) {
        if(PageManager.HomePage == PageManager.PublicDown) {
            if(PageManager.PublicDown->Up.Page->obj) {
                lv_obj_add_flag(PageManager.PublicDown->Up.Page->obj, LV_OBJ_FLAG_HIDDEN);
            }   
        }
    }
    ...
    if(PageManager.HomePage->id == ID_SCREEN_VOLUME) {
        if(no_lantacy_state == 0) {
            no_lantacy_state = 1;
            if(PM_Custom_Cb.ble_setting_cmd_send) {
...
void PM_Init(void)
{
    ...
    PageManager.Bobj = lv_obj_create(PageManager.obj);
    lv_obj_add_style(PageManager.Bobj, &PageManager.DefaultStyle, 0);
    lv_obj_set_style_bg_opa(PageManager.Bobj, LV_OPA_TRANSP, 0);
    lv_obj_set_size(PageManager.Bobj, WIDTH, HEIGHT);
    lv_obj_clear_flag(PageManager.Bobj, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_clear_flag(PageManager.Bobj, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(PageManager.Bobj, LV_OBJ_FLAG_EVENT_BUBBLE);
    lv_obj_set_scrollbar_mode(PageManager.Bobj, LV_SCROLLBAR_MODE_OFF);

    PageManager.Mobj = lv_obj_create(PageManager.obj);
    lv_obj_add_style(PageManager.Mobj, &PageManager.DefaultStyle, 0);
    lv_obj_set_style_bg_opa(PageManager.Mobj, LV_OPA_TRANSP, 0);
    lv_obj_set_size(PageManager.Mobj, WIDTH, HEIGHT);
    lv_obj_clear_flag(PageManager.Mobj, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_clear_flag(PageManager.Mobj, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(PageManager.Mobj, LV_OBJ_FLAG_EVENT_BUBBLE);
    lv_obj_set_scrollbar_mode(PageManager.Mobj, LV_SCROLLBAR_MODE_OFF);

    PageManager.Tobj = lv_obj_create(PageManager.obj);
    lv_obj_add_style(PageManager.Tobj, &PageManager.DefaultStyle, 0);
    lv_obj_set_style_bg_opa(PageManager.Tobj, LV_OPA_TRANSP, 0);
    lv_obj_set_size(PageManager.Tobj, WIDTH, HEIGHT);
    lv_obj_clear_flag(PageManager.Tobj, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_clear_flag(PageManager.Tobj, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_add_flag(PageManager.Tobj, LV_OBJ_FLAG_EVENT_BUBBLE);
    lv_obj_set_scrollbar_mode(PageManager.Tobj, LV_SCROLLBAR_MODE_OFF);
...
void PM_ClosePage(PageTypeHandle* SourcePage, enum PageDir Dir)
{
    ...
    if(Page != NULL)
    {
        if(Page->State == VALID_AND_CREATED)
        {
            if(PageManager.PublicDown != NULL) {
                // 公共下页面不释放
                if(Page != PageManager.PublicDown) {
...
{
    if((SourcePage == NULL) | (SourcePage->State == INVALID))
    {
        DEBUG("SourcePage Is No Add To PageManager\r\n");
        return;
    }

    if((TargetPage == NULL) | (TargetPage->State == INVALID))
    {
...
{
    DEBUG("Page Enter, id: %d, state: %d", Page->id, Page->State);
    // if(PageManager.HomePage != NULL) {
    //     PM_ClosePage(PageManager.HomePage, PageUp);
    // }
    if(Page != NULL) {
        PM_ClosePage(Page, PageUp);
        PM_ClosePage(Page, PageDown);
        PM_ClosePage(Page, PageLeft);
        PM_ClosePage(Page, PageRight);   

        if(PageManager.PublicDown != NULL && PageManager.PublicDown->obj != NULL) {
            // 页面跳转时下页面往下挪     
            if(lv_obj_get_y(PageManager.PublicDown->obj) != -lv_obj_get_height(PageManager.obj)) {
                lv_obj_set_pos(PageManager.PublicDown->obj, 0, -lv_obj_get_height(PageManager.obj));
            }
        }
...
void PM_SetHomePage(PageTypeHandle* Page)
{
    ...
    switch (Page->State)
    {
        ...
        case VALID:{
            ...
            if(PageBuf->Layer == LAYER_TOP) {
                PageBuf->obj = lv_obj_create(PageManager.Tobj);
            } else if(PageBuf->Layer == LAYER_MIDDLE) {
                PageBuf->obj = lv_obj_create(PageManager.Mobj);
            } else {
                PageBuf->obj = lv_obj_create(PageManager.Bobj);
            }
            ...
            PageBuf->State = VALID_AND_CREATED;
        ...
        case VALID_AND_CREATED:{
            if(PageManager.HomePage != NULL)
            {
                // Page的VAILD_AND_CREATED状态说明与HomePage相邻
                // HomePage若存在，则清除除了自己的其他旧页面
                b_printf("homepage.id: %d", PageManager.HomePage->id);
                DEBUG("Clear Old HomePage But Me\r\n");
                enum PageState StateBuf = GetDirState(PageManager.HomePage, PageUp);
                b_printf("StateBuf: %d, Up.Page: %d, down.page: %d, left.page: %d, right.page: %d", StateBuf, PageManager.HomePage->Up.Page->id, PageManager.HomePage->Down.Page->id, PageManager.HomePage->Left.Page->id, PageManager.HomePage->Right.Page->id);
                p_printf("Page.id: %d, u, d, l, r: %d %d %d %d", Page->id, Page->Up.Page->id, Page->Down.Page->id, Page->Left.Page->id, Page->Right.Page->id);
                if((StateBuf == VALID_AND_CREATED) & (PageManager.HomePage->Up.Page != Page))
                {
                ...
                if((StateBuf == VALID_AND_CREATED) & (PageManager.HomePage->Down.Page != Page))
                {
                ...
                if((StateBuf == VALID_AND_CREATED) & (PageManager.HomePage->Left.Page != Page))
                {
                ...
                if((StateBuf == VALID_AND_CREATED) & (PageManager.HomePage->Right.Page != Page))
                {
    ...
    if(Page->Up.Mode != LOAD_ANIM_NONE)
    {
        ...
        {
            case INVALID:{
                DEBUG("HomePage's UpPage Is INVALID\r\n");
            }break;

            case VALID:{
                DEBUG("HomePage's UpPage Is VALID, Id: %d\r\n", PageBuf->id);
                if(PageBuf->Layer == LAYER_TOP) {
                    PageBuf->obj = lv_obj_create(PageManager.Tobj);
                } else if(PageBuf->Layer == LAYER_MIDDLE) {
                    PageBuf->obj = lv_obj_create(PageManager.Mobj);
            ...
            case VALID_AND_CREATED:{
                DEBUG("HomePage's UpPage Is Create, ID: %d\r\n", Page->Up.Page->id);
                PageManager.Dir |= LV_DIR_TOP;
    ...
    if(Page->Down.Mode != LOAD_ANIM_NONE)
    {
        ...
        {
            case INVALID:{
                DEBUG("HomePage's DownPage Is INVALID\r\n");
            }break;

            case VALID:{
                DEBUG("HomePage's DownPage Is VALID, Id: %d\r\n", PageBuf->id);
                if(PageBuf->Layer == LAYER_TOP) {
                    PageBuf->obj = lv_obj_create(PageManager.Tobj);
                } else if(PageBuf->Layer == LAYER_MIDDLE) {
                    PageBuf->obj = lv_obj_create(PageManager.Mobj);
            ...
            case VALID_AND_CREATED:{
                DEBUG("HomePage's DownPage Is Create, Id: %d\r\n", Page->Down.Page->id);
                PageManager.Dir |= LV_DIR_BOTTOM;
    ...
    if(Page->Left.Mode != LOAD_ANIM_NONE)
    {
        ...
        {
            case INVALID:{
                DEBUG("HomePage's LeftPage Is INVALID\r\n");
            }break;

            case VALID:{
                DEBUG("HomePage's LeftPage Is VALID, Id: %d\r\n", PageBuf->id);
                if(PageBuf->Layer == LAYER_TOP) {
                    PageBuf->obj = lv_obj_create(PageManager.Tobj);
                } else if(PageBuf->Layer == LAYER_MIDDLE) {
                    PageBuf->obj = lv_obj_create(PageManager.Mobj);
            ...
            case VALID_AND_CREATED:{
                DEBUG("HomePage's LeftPage Is Create, Id: %d\r\n", Page->Left.Page->id);
                PageManager.Dir |= LV_DIR_LEFT;
                if(Page->Left.Mode == LOAD_ANIM_OVER || Page->Left.Mode == LOAD_ANIM_LEAVE) {
#if PAGE_MOVE_COVER_MODE
                    lv_obj_move_foreground(PageBuf->obj);
                    // lv_obj_move_to_index(Page->obj, lv_obj_get_index(Page->obj) - 1);
#endif
                    break;
                }
                lv_obj_align_to(PageBuf->obj, Page->obj, LV_ALIGN_OUT_LEFT_MID, 0, 0);
    ...
    if(Page->Right.Mode != LOAD_ANIM_NONE)
    {
        ...
        {
            case INVALID:{
                DEBUG("HomePage's RightPage Is INVALID\r\n");
            }break;

            case VALID:{
                DEBUG("HomePage's RightPage Is VALID, Id: %d\r\n", PageBuf->id);
                if(PageBuf->Layer == LAYER_TOP) {
                    PageBuf->obj = lv_obj_create(PageManager.Tobj);
                } else if(PageBuf->Layer == LAYER_MIDDLE) {
                    PageBuf->obj = lv_obj_create(PageManager.Mobj);
            ...
            case VALID_AND_CREATED:{
                DEBUG("HomePage's RightPage Is Create, Id: %d\r\n", Page->Right.Page->id);
                PageManager.Dir |= LV_DIR_RIGHT;
    ...
    if(PageManager.PublicDown != NULL) {
        // 动态绑定下页面跟的上页面
        if(Page != PageManager.PublicDown) {
            SetDirPage(PageManager.PublicDown, PageUp, Page);
            SetDirMode(PageManager.PublicDown, PageUp, LOAD_ANIM_LEAVE);
        }

        if(lv_obj_has_flag(Page->obj, LV_OBJ_FLAG_HIDDEN)) {
