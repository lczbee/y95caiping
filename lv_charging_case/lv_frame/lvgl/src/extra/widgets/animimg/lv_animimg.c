...
void lv_animimg_start(lv_obj_t * obj)
{
    ...
    if(animimg2->img.src){
        if(strcmp("RES", lv_fs_get_ext(animimg2->img.src))==0){
            extern void low_rle_anim_start3(lv_obj_t * obj, int cnt, int enddly, lv_anim_start_cb_t start_cb, lv_anim_ready_cb_t ready_cb);
            low_rle_anim_start3(obj, 0, 0, 0, 0);
            g_printf("lv_animimg_start %s", animimg2->img.src);
            return;
        }         
    }
